...
@available(iOS 15.0, *)
public extension View {
    func withOldNavigation<C: NavigationCoordinator>(_ coordinator: C) -> some View {
        NavigationWrapper(content: self, coordinator: coordinator)
    }
}

@available(iOS 15.0, *)
private struct NavigationWrapper<Content: View, Coordinator: NavigationCoordinator>: View {

    let content: Content
    @ObservedObject var state: NavigationState
    let coordinator: Coordinator

    init(content: Content, coordinator: Coordinator) {
    ...
    private func nextDestination() -> some View {
        if let screen = state.path.last as? Coordinator.Screen {
            coordinator.destination(for: screen)
        } else {
            EmptyView()
        }
...
@MainActor
public extension NavigationCoordinator {
    func oldView(for screen: Screen) -> some View {
        destination(for: screen)
            .withOldNavigation(self)
            .withModal(self)
    }
}
